...
IERC165
{
   ...
   */
    ...
    /// @notice Address of the vault where migration of debt and collateral will take place
   ...
   */
    ...
    /// @notice An event emitted when the migration contract is updated
  ...
  /**
   * @notice Sets the vault contract address where migration will happen
   * @param _migrationVault address
   * @dev Only owner can call it
   */
	function setMigrationVault(address _migrationVault) external virtual onlyOwner {
        require(
        IVaultHandler(_migrationVault).divisor() == divisor &&
            IVaultHandler(_migrationVault).ratio() == ratio &&
            IVaultHandler(_migrationVault).burnFee() == burnFee &&
            IVaultHandler(_migrationVault).liquidationPenalty() ==
            liquidationPenalty &&
            IVaultHandler(_migrationVault).tcapOracle() == tcapOracle &&
            IVaultHandler(_migrationVault).collateralContract() ==
            collateralContract &&
            IVaultHandler(_migrationVault).collateralPriceOracle() ==
            collateralPriceOracle &&
            IVaultHandler(_migrationVault).ETHPriceOracle() == ETHPriceOracle &&
            IVaultHandler(_migrationVault).TCAPToken() == TCAPToken &&
            IVaultHandler(_migrationVault).treasury() == treasury &&
            IVaultHandler(_migrationVault).minimumTCAP() == minimumTCAP,
        "VaultHandler::setMigrationVault: vault configuration doesn't match"
        );
        require(TCAPToken.vaultHandlers(_migrationVault),"VaultHandler::setMigrationVault: vault still not added as a Cryptex Vault");
        migrationVault = _migrationVault;
		emit NewMigrationVault(msg.sender, _migrationVault);
	}
